...
- **2026-07-26 - Milestone 2 Roadmap**: Planned and initialized issues 10–13 covering Two-Tier Zero-Token Job URL Auto-Extraction (`JobUrlParserService` via OpenGraph/Schema.org with Gemini AI fallback), Philippine Statutory Tax & Net Take-Home Pay Calculator (`PhilippineTaxCalculatorService`), Offer Comparison Matrix Page (`/job-applications/offers`), and User Settings & Theme Management (`/settings`).