...
import { User, Briefcase, GraduationCap, Wrench, Award, FileText, Download, Mail } from 'lucide-react';
...
function PersonalInfoTab({ data, setData, errors }: { data: ResumeProfile; setData: (key: string, value: string) => void; errors: Record<string, string> }) {
    return (
...
function ResumePreview({ data, template }: { data: ResumeProfile; template: string }) {
            ...
            <head>
                ...
                <style>
                    ...
                    * { margin: 0; padding: 0; box-sizing: border-box; }
                    body { font-family: 'Instrument Sans', sans-serif; color: #1b1b18; padding: 40px; max-width: 800px; margin: 0 auto; }
                    ...
                    .contact { font-size: 13px; color: #706f6c; margin-bottom: 16px; }
                    .summary { font-size: 14px; line-height: 1.5; margin-bottom: 16px; }
                    .job, .edu { margin-bottom: 12px; }
                    .job-header, .edu-header { display: flex; justify-content: space-between; font-size: 14px; font-weight: 500; }
        ...
        return (
            <div className="flex flex-col gap-4">
                <div ref={previewRef} className="rounded-lg border border-[#e3e3e0] bg-white p-6 text-[#1b1b18] shadow-sm dark:border-[#3E3E3A] dark:bg-[#161615] dark:text-[#EDEDEC]">
                    <h1 className="text-2xl font-semibold">{data.full_name || 'Your Name'}</h1>
                    ...
                    {data.linkedin_url && (
                        <div className="mt-1 text-xs text-[#706f6c]">{data.linkedin_url}</div>
                    )}
                            ...
                            {data.work_experience?.map((job, i) => (
                                <div key={i} className="mb-3">
                                    <div className="flex justify-between text-sm font-medium">
                                        ...
                                        <span className="text-xs text-[#706f6c]">{job.duration}</span>
                                    </div>
                                    <div className="text-xs text-[#706f6c]">{job.company}</div>
                                    {job.description && (
                                        <p className="mt-1 text-xs leading-relaxed">{job.description}</p>
                                    )}
                                </div>
                            ))}
                        </>
                            ...
                            {data.education?.map((edu, i) => (
                                <div key={i} className="mb-2">
                                    <div className="flex justify-between text-sm font-medium">
                                        ...
                                        <span className="text-xs text-[#706f6c]">{edu.year}</span>
                                    </div>
                                    <div className="text-xs text-[#706f6c]">{edu.institution}</div>
                                </div>
                            ))}
                        </>
                    )}
                        ...
                        <>
                            ...
                            <div className="flex flex-wrap gap-1.5">
                                {data.skills?.map((skill) => (
                                    <span key={skill} className="rounded-full bg-[#f5f5f4] px-2.5 py-0.5 text-xs text-[#706f6c] dark:bg-[#1C1C1A] dark:text-[#A1A09A]">
                                        {skill}
                                    </span>
                                ))}
                            </div>
        ...
        return (
            <div className="flex flex-col gap-4">
                <div ref={previewRef} className="rounded-lg border border-[#e3e3e0] bg-white shadow-sm dark:border-[#3E3E3A] dark:bg-[#161615]">
                    <div className="bg-[#1b1b18] p-6 text-white dark:bg-[#EDEDEC] dark:text-[#1b1b18]">
                        ...
                        {data.linkedin_url && (
                            <div className="mt-1 text-xs opacity-75">{data.linkedin_url}</div>
                        )}
                    </div>
                    ...
                    <div className="p-6">
                        {data.summary && (
                            <div className="mb-4">
                                <h2 className="mb-2 text-xs font-semibold uppercase tracking-wider text-[#706f6c] dark:text-[#A1A09A]">Summary</h2>
                                <p className="text-sm leading-relaxed">{data.summary}</p>
                            </div>
                        )}
                        ...
                        {(data.work_experience?.length ?? 0) > 0 && (
                            <div className="mb-4">
                                <h2 className="mb-2 text-xs font-semibold uppercase tracking-wider text-[#706f6c] dark:text-[#A1A09A]">Work Experience</h2>
                                {data.work_experience?.map((job, i) => (
                                    <div key={i} className="mb-3 border-l-2 border-[#1b1b18] pl-3 dark:border-[#EDEDEC]">
                                        <div className="flex justify-between text-sm font-medium">
                                            <span>{job.position || 'Position'}</span>
                                            <span className="text-xs text-[#706f6c] dark:text-[#A1A09A]">{job.duration}</span>
                                        </div>
                                        <div className="text-xs text-[#706f6c] dark:text-[#A1A09A]">{job.company}</div>
                                        {job.description && (
                                            <p className="mt-1 text-xs leading-relaxed">{job.description}</p>
                                        )}
                                    </div>
                        ...
                        {(data.education?.length ?? 0) > 0 && (
                            <div className="mb-4">
                                <h2 className="mb-2 text-xs font-semibold uppercase tracking-wider text-[#706f6c] dark:text-[#A1A09A]">Education</h2>
                                {data.education?.map((edu, i) => (
                                    <div key={i} className="mb-2">
                                        <div className="flex justify-between text-sm font-medium">
                                            <span>{edu.degree || 'Degree'}</span>
                                            <span className="text-xs text-[#706f6c] dark:text-[#A1A09A]">{edu.year}</span>
                                        </div>
                                        <div className="text-xs text-[#706f6c] dark:text-[#A1A09A]">{edu.institution}</div>
                                    </div>
                                ))}
                            </div>
                        )}
                        ...
                        {(data.skills?.length ?? 0) > 0 && (
                            <div className="mb-4">
                                <h2 className="mb-2 text-xs font-semibold uppercase tracking-wider text-[#706f6c] dark:text-[#A1A09A]">Skills</h2>
                                <div className="flex flex-wrap gap-1.5">
                                    {data.skills?.map((skill) => (
                                        <span key={skill} className="rounded-full bg-[#f5f5f4] px-2.5 py-0.5 text-xs text-[#706f6c] dark:bg-[#1C1C1A] dark:text-[#A1A09A]">
                                            {skill}
                        ...
                        {(data.certifications?.length ?? 0) > 0 && (
                            <div>
                                <h2 className="mb-2 text-xs font-semibold uppercase tracking-wider text-[#706f6c] dark:text-[#A1A09A]">Certifications</h2>
                                <div className="text-xs leading-relaxed">
                                    {data.certifications?.join(' | ')}
                                </div>
                            </div>
    ...
    return (
        <div className="flex flex-col gap-4">
            <div ref={previewRef} className="rounded-lg border border-[#e3e3e0] bg-white p-6 text-[#1b1b18] shadow-sm dark:border-[#3E3E3A] dark:bg-[#161615] dark:text-[#EDEDEC]">
                <div className="text-center border-b-2 border-[#1b1b18] pb-4 dark:border-[#EDEDEC]">
                    <h1 className="text-2xl font-bold uppercase tracking-wide">{data.full_name || 'Your Name'}</h1>
                    <div className="mt-2 text-xs text-[#706f6c]">
                        {data.location && <span>{data.location}</span>}
                        {data.phone && <span> | Tel: {data.phone}</span>}
                    </div>
                    {data.email && (
                        <div className="text-xs text-[#706f6c]">Email: {data.email}</div>
                    )}
                    {data.linkedin_url && (
                        <div className="mt-1 text-xs text-[#706f6c]">LinkedIn: {data.linkedin_url}</div>
                    )}
                </div>
                ...
                {data.summary && (
                    <div className="mt-4">
                        ...
                        <p className="mt-1 text-xs leading-relaxed">{data.summary}</p>
                    </div>
                )}

                {(data.work_experience?.length ?? 0) > 0 && (
                    <div className="mt-4">
                        <h2 className="text-sm font-bold uppercase">Work Experience</h2>
                        <div className="mt-1 border-t border-[#e3e3e0] dark:border-[#3E3E3A]" />
                        {data.work_experience?.map((job, i) => (
                            <div key={i} className="mt-3">
                                <div className="text-sm font-semibold">{job.company || 'Company'}</div>
                                <div className="flex justify-between text-xs">
                                    <span className="italic">{job.position}</span>
                                    <span className="text-[#706f6c]">{job.duration}</span>
                                </div>
                                {job.description && (
                                    <p className="mt-1 text-xs leading-relaxed">{job.description}</p>
                                )}
                        ...
                        {data.education?.map((edu, i) => (
                            <div key={i} className="mt-2">
                                <div className="flex justify-between text-xs">
                                    ...
                                    <span className="text-[#706f6c]">{edu.year}</span>
                                </div>
                                <div className="text-xs text-[#706f6c]">{edu.institution}</div>
                            </div>
                        ))}
...
export default function DocumentsIndex({ profile }: DocumentsPageProps) {
                            ...
                            <Card>
                                ...
                                <CardContent className="flex flex-col gap-4">
                                    ...
                                    <div className="min-h-[300px]">
                                        {activeTab === 'personal' && <PersonalInfoTab data={data} setData={setData} errors={errors} />}
